...
int jsmn_link_list(jsmntok_t *tokens) {
    tokens->value = 0;
    tokens->next = tokens->prev = tokens;
    if (tokens->type == JSMN_STRING) {
        jsmntok_t *ctok = tokens+1;
        ctok->value = 0;
        int cnt = 0;
        tokens->value = tokens+1;
    ...
    if (tokens->type == JSMN_OBJECT){
        ...
        ctok->prev->next = tokens->value;
        tokens->value->prev = ctok->prev;
        return cnt;
...
#ifdef JSMN_LINKED_LIST
    if (count > 0 && tokens && jsmn_link_list(tokens) < 0) return JSMN_ERROR_LINKING;
#endif
